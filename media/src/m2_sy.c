...
// recolour enemy player
  call(METH_VIEW, MS_OB_VIEW_COLOUR_PROC, 1, // player 1
							80, 240, // min and max red intensity
							80, 50, // min and max g intensity
							210, 40); // min and max b intensity
  call(METH_VIEW, MS_OB_VIEW_COLOUR_PACKET, 1, // player 1
							250, 130, // min and max red intensity
							40, 100, // min and max g intensity
							40, 240); // min and max b intensity
...
// Now set up templates for player 1 (the attacker)
  call(METH_TEMPLATE, TEMPLATE_P1_PROCESS_0, process_start(auto_factory), process_end(auto_factory), &process_name [0] [0]);
  call(METH_TEMPLATE, TEMPLATE_P1_PROCESS_1, process_start(attacker), process_end(attacker), &process_name [1] [0]);
  call(METH_TEMPLATE, TEMPLATE_P1_PROCESS_2, process_start(T_attacker), process_end(T_attacker), &process_name [2] [0]);
...
// Now, place a process for the player:
   ...
   spawn_enemy(TEMPLATE_P1_PROCESS_0, world_size_x - 1700, world_size_y / 4, ANGLE_2);
// auto factories automatically build from template 1.
// Here we'll tell the middle one to build from template 2 instead:
   int middle_factory;
   middle_factory = spawn_enemy(TEMPLATE_P1_PROCESS_0, world_size_x - 700, world_size_y / 2, ANGLE_2);
			command(middle_factory, COMREG_QUEUE, COMMAND_BUILD);
			command(middle_factory, COMREG_QUEUE + 1, 2);
   command_bit_1(middle_factory, COMREG_CLIENT_STATUS, COM_CLIENT_ST_NEW);
   spawn_enemy(TEMPLATE_P1_PROCESS_0, world_size_x - 1700, (world_size_y / 4) * 3, ANGLE_2);
   break;
