...
mini-spider that escorts the main spider.
Should be fed location of main spider by delegate.
...
interface
{
 PROGRAM_TYPE_PROCESS, SHAPE_4DIAMOND, 2, 2, // program type, shape, size (from 0-3), base vertex
 {
  METH_MOVE1: {MT_PR_MOVE, 1, ANGLE_2 - data(DATA_SHAPE_VERTEX_ANGLE, SHAPE_4SQUARE, 1)}, // acceleration method on vertex 1, pointing backwards
  METH_MOVE2: {MT_PR_MOVE, 3, ANGLE_2 - data(DATA_SHAPE_VERTEX_ANGLE, SHAPE_4SQUARE, 3)}, // acceleration method on vertex 3, pointing backwards
  METH_MATHS: {MT_PR_MATHS}, // maths method. allows trigonometry etc.
  METH_STD: {MT_PR_STD}, // standard method. among other things, allows access to process's current properties (location etc)
  METH_IRPT: {MT_PR_IRPT, 0, 0, 0}, // generates irpt. functions automatically (although can be configured not to)
  METH_PACKET: {MT_PR_PACKET, 0, 0, 1, 1}, // packet method. allows process to attack
...
enum
{
MODE_FOLLOW, // Process is following main spider
...
// What the process does next depends on which mode it is in.
 ...
 {
  case MODE_FOLLOW:
  	spider_x = get_command(0);
  	spider_y = get_command(1);
   spider_distance = get_command(2);
   if (spider_distance < 800)
				spider_distance = 800;
//  	target_x = spider_x + cos(spider_angle, spider_distance);
//  	target_y = spider_y + sin(spider_angle, spider_distance);
//  	spider_angle += 70;
   int angle_from_spider;
  	angle_from_spider = atan2(y - spider_y, x - spider_x);
  	target_x = spider_x + cos(angle_from_spider + 500, spider_distance);
  	target_y = spider_y + sin(angle_from_spider + 500, spider_distance);
   scan_for_target();
// In FOLLOW mode, the process goes to a location near the main spider.
   break;
...
// If the target is (roughly) within 600 pixels of this process, consider firing at it:
    if (abs(attack_y) + abs(attack_x) < 600) // Could use hypot(), but that's more expensive
    {
...
// Now, if the process is pointing more or less towards its target, fire!
...
// Make sure we've got enough irpt to fire:
       if (get_irpt() > 300) // get_irpt() is a built-in INFO function.
        put(METH_PACKET, MB_PR_PACKET_COUNTER, 1);
