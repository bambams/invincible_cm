...
This is a very simple process that wanders around looking for a target, then attacks it.
...
interface
{
 PROGRAM_TYPE_PROCESS, SHAPE_5LONG, 3, 0, // program type, shape, size (from 0-3), base vertex
 {
  METH_MOVE1: {MT_PR_MOVE, 4, 0 - data(DATA_SHAPE_VERTEX_ANGLE, SHAPE_5LONG, 4)}, // acceleration method on vertex 3, pointing backwards
  METH_MOVE2: {MT_PR_MOVE, 1, 0 - data(DATA_SHAPE_VERTEX_ANGLE, SHAPE_5LONG, 1)}, // acceleration method on vertex 1, pointing backwards
  ...
  METH_PACKET1: {MT_PR_PACKET, 2, ANGLE_2 - data(DATA_SHAPE_VERTEX_ANGLE, SHAPE_5LONG, 2) + 120, 0, 1}, // packet method. allows process to attack
  METH_PACKET2: {MT_PR_PACKET, 3, ANGLE_2 - data(DATA_SHAPE_VERTEX_ANGLE, SHAPE_5LONG, 3) - 120, 0, 1}, // packet method. allows process to attack
...
// Make sure we've got enough irpt to fire:
       if (get_irpt() > 300) // get_irpt() is a built-in INFO function.
							{
        put(METH_PACKET1, MB_PR_PACKET_COUNTER, 1);
        put(METH_PACKET2, MB_PR_PACKET_COUNTER, 9);
							}
