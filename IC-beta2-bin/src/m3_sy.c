...
process enemy_factory;
process double_attacker;
process attacker;
process auto_factory;
...
// recolour enemy player
  call(METH_VIEW, MS_OB_VIEW_COLOUR_PROC, 1, // player 1
							180, 240, // min and max red intensity
							150, 50, // min and max g intensity
							110, 40); // min and max b intensity
  call(METH_VIEW, MS_OB_VIEW_COLOUR_PACKET, 1, // player 1
							190, 240, // min and max red intensity
							80, 240, // min and max g intensity
							10, 120); // min and max b intensity
  call(METH_VIEW, MS_OB_VIEW_COLOUR_DRIVE); // sets drive to same as packet (all registers except 0 retain their values)
...
// Now set up templates for player 1 (the attacker)
  call(METH_TEMPLATE, TEMPLATE_P1_PROCESS_0, process_start(enemy_factory), process_end(enemy_factory), &process_name [0] [0]);
  call(METH_TEMPLATE, TEMPLATE_P1_PROCESS_1, process_start(double_attacker), process_end(double_attacker), &process_name [1] [0]);
  call(METH_TEMPLATE, TEMPLATE_P1_PROCESS_2, process_start(attacker), process_end(attacker), &process_name [2] [0]);
  call(METH_TEMPLATE, TEMPLATE_P1_PROCESS_3, process_start(auto_factory), process_end(auto_factory), &process_name [3] [0]);
...
// Okay, pregame is over:
   ...
   print("\n\nA malicious intrusion has been detected!");
   print("\nSomeone is trying to take control of your system.");
...
// Now, place a process for the player:
   ...
   spawn_enemy(TEMPLATE_P1_PROCESS_0, world_size_x - 1200, world_size_y / 2, ANGLE_2);
   int new_process_index;
   new_process_index = spawn_enemy(TEMPLATE_P1_PROCESS_3, world_size_x - 1900, world_size_y / 4, ANGLE_2 - ANGLE_16);
			command(new_process_index, COMREG_QUEUE, COMMAND_BUILD);
			command(new_process_index, COMREG_QUEUE + 1, 2);
   command_bit_1(new_process_index, COMREG_CLIENT_STATUS, COM_CLIENT_ST_NEW);
   new_process_index = spawn_enemy(TEMPLATE_P1_PROCESS_3, world_size_x - 1900, (world_size_y / 4) * 3, ANGLE_2 - ANGLE_16);
			command(new_process_index, COMREG_QUEUE, COMMAND_BUILD);
			command(new_process_index, COMREG_QUEUE + 1, 2);
   command_bit_1(new_process_index, COMREG_CLIENT_STATUS, COM_CLIENT_ST_NEW);
   break;
