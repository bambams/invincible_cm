...
int add_text_at_end_of_line(struct source_editstruct* se, char* add_str);
int add_text_at_start_of_line(struct source_editstruct* se, char* add_str);
...
void remove_text_from_source_line(struct source_editstruct* se, int sline, int text_end, int chars);
void insert_text_into_source_line(struct source_editstruct* se, int sline, int target_start, int buffer_pos, int chars);
void insert_text_into_source_line_backwards(struct source_editstruct* se, int sline, int target_start, int buffer_pos, int chars);
//void pu(void);
...
// find out if we need to put a new entry on the undo stack:
 if (editor.undo_type [editor.undo_pos] != UNDO_TYPE_INSERT_TEXT
  || editor.undo_se [editor.undo_pos] != editor.tab_index [editor.current_tab] // if there's no current tab get_current_source_edit will already have failed
...
//   fprintf(stdout, "\nNow %i (base %i)", editor.undo_pos, editor.undo_pos_base);
  ...
  editor.undo_se [editor.undo_pos] = editor.tab_index [editor.current_tab];
...
/* fprintf(stdout, "\n(%i)", editor.undo_buffer [0]);
 ...
 if (editor.undo_type [editor.undo_pos] == UNDO_TYPE_NONE
  ...
  || editor.undo_se [editor.undo_pos] != get_current_source_edit_index())
  return;
...
void call_redo(void)
{
 ...
 if (editor.undo_type [editor.undo_pos + 1] == UNDO_TYPE_NONE
  ...
  || editor.undo_se [editor.undo_pos + 1] != get_current_source_edit_index())
  return;
