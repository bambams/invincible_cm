...
// First save contents of gamestruct (although not all values need to be saved, which is why we don't use save_gamestruct_to_file() in f_save.c)
// save_int(game.phase);
// save_int(game.turns);
...
// Now save contents of w_init (which should have been set up by a call to derive_world_init_from_menu() in s_menu.c)
 ...
 save_int(w_init.procs_per_team);
// save_int(w_init.gen_limit);
 save_int(w_init.packets_per_team);
...
// other gamestruct values are filled in by start_game
...
// set up some basic game values from the initstruct:
 ...
 load_int(&w_init.procs_per_team, MIN_PROCS_PER_TEAM, MAX_PROCS_PER_TEAM, "processes per team");
// load_int(&w_init.gen_limit, MIN_GEN_LIMIT, MAX_GEN_LIMIT, "gen limit");
 load_int(&w_init.packets_per_team, MIN_PACKETS, MAX_PACKETS, "packets");
